...
func (dataSourceData *TSforDatasource) GetData(tags []string, fromTimestamp uint64, toTimestamp uint64) map[string]map[uint64]float64 {
	...
	for _, tag := range tags {
		if dataSourceData.contains(tag) {
			...
			ans[tag] = make(map[uint64]float64) //TODO: throw exception?
		}
	}
	return ans
}

func (dataSourceData *TSforDatasource) SaveData(data map[string]map[uint64]float64, expiration uint64) {
