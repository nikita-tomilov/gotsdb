...
func TestInMemTSS_BasicFunctionsWork2(t *testing.T) {
	...
	//then
	assert.Equal(t, make(map[uint64]float64), retrievedData[testTag1].Points, "Data for tag1 for 05.05-06.05 should be empty")
	assert.Equal(t, dataToStore[testTag2], retrievedData[testTag2], "Data for tag2 for 05.05-06.05 should be same")
}
